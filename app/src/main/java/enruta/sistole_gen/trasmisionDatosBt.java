...
//							c.moveToFirst();
							bytesAEnviar=globales.tdlg.encabezadoAMandar(db);
//							c.close();
			   			...
			   			mostrarMensaje(PROGRESO,  getString(R.string.msj_trans_generando_no_registrados));
			   				...
			   				if (cantidad>0){
								...
								dos.flush();
			   					...
			   					for (int i=0;i<cantidad;i++){
				   					...
				   					bytesAEnviar=(c.getString(c.getColumnIndex("poliza")) + "\r\n").getBytes("ISO-8859-1");
							...
							dos.flush();
			   				...
			   				for (int archivo: globales.tdlg.getArchivosATransmitir()){
			   				//Aqui enviamos los no registrados
					   			mostrarMensaje(PROGRESO,  globales.tdlg.regresaMensajeDeTransmision(archivo));
					   				...
					   				if (cantidad>0){
					   					...
					   					dos.writeInt(globales.tdlg.opcionAElegir(archivo));
										dos.flush();
										...
										dos.flush();
					   					...
					   					for (int i=0;i<cantidad;i++){
						   					...
						   					bytesAEnviar=(c.getString(c.getColumnIndex("texto")) + "\r\n").getBytes("ISO-8859-1");
				...
				// TODO Auto-generated method stub
		   		...
		   		int i=0, nStatusDeRecepcion;
		   	        ...
		   	        try {
		   	        	...
		   	        	lby_cadena=  globales.tdlg.encabezadoAEnviar(ls_carpeta, ls_categoria).getBytes();
		   			 ...
		   			 borrarRuta(db);
		   				...
		   				for ( i=0; i<totalDeMedidoresALeer;i++){
		   					...
		   					if (globales.desencriptarEntrada)
		   					{
		   						globales.tdlg.EncriptarDesencriptar(lby_cadena);
		   					}
			   				...
			   				if (i!=0 && !ls_linea.startsWith("#") && !ls_linea.startsWith("!") && !globales.tdlg.esUnRegistroRaro(ls_linea)){
			   					secuenciaReal++;
